...
export async function startTradingBot(config: BotConfig, botDoc: any) {
  try {
    ...
    // 🔥 AUTH FIX (only change)
    const authId = botDoc.authId;
    ...
    if (botDoc.runtime) {
      state = { ...state, ...botDoc.runtime };
    }

    async function persistRuntime() {
      await BotModel.findByIdAndUpdate(botDoc.id, { runtime: state });
      ...
      try {
        ...
        if (state.inPosition && state.entryPrice != null) {
          const qty = Number(config.configuration?.quantity ?? 0);
          try {
            const fetchSymbol = apiSymbol.replace(/USD$/, "USDT");
            const candles = await fetchCandlesFromBinance(fetchSymbol, "1m", 1);
            const latest = Number(candles?.[candles.length - 1]?.close ?? 0);
            console.log("debug latest =====> ", latest);
      ...
      while (!stopped) {
        try {
          ...
          if (
            ...
            if (filled > 0 && state.entryPrice != null) {
              const contractSize = await computeContractSize(apiSymbol, price);
              const pnl =
                (price - state.entryPrice) * filled * contractSize;
              ...
              console.log(
                `[BOT ${botDoc.id}] SELL FILLED | price=${price} | qty=${quantity}`
              );
